...
import AddEditSubSubjectModel from "./AddEditSubSubjectModel";
...
export function SubSubjectTable() {
  const [editSubSubjectId, setEditSubSubjectId] = useState<string | null>(null);
  const [subSubjectData, setSubSubjectData] = useState([]);

  const [open, setOpen] = useState(false);
  const { setLoader } = useLoaderContext();

  const SubSubjectColumns: TableColumn<SubjectType>[] = [
    {
      name: "Id",
      selector: (row) => row.id,
  ...
  async function getData() {
    ...
    try {
      ...
      if (responseSub.status == 200) {
        const data = responseSub.data.result;
  ...
  return (
    <Container className="m-4 border-2 min-h-[70vh] p-4 ">
      <Flex justify={"between"} className="m-[2rem]">
        {" "}
        <Text className="text-3xl font-normal">Sub Subject</Text>
        <AddEditSubSubjectModel
          open={open}
          setOpen={setOpen}
          editSubSubjectId={editSubSubjectId}
        />
      </Flex>
      <DataTable
        columns={SubSubjectColumns}
        data={subSubjectData}
        paginationPerPage={5}
